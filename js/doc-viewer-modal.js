/**
 * ARUZ CORE 360 - INTERACTIVE DOCUMENT VIEWER MODAL ("REACT-STYLE" SPA VIEWER)
 * Displays Architectural Blueprints, Technical Datasheets, Brochures, and Sales Terms
 * seamlessly in-page without leaving the site or forcing raw PDF downloads.
 */
...
(function () {
  ...
  const PROPERTY_DOCS = {
    ...
    'sak-luum': {
      ...
      specs: [
        ...
        { label: 'Cimentación', val: 'Zapata Corrida CADE f\'c=250' },
      ...
      terms: [
        ...
        { title: 'Pagos a Obra', desc: '60% diferido según avance físico.' },
        { title: 'Entrega', desc: '10% contra entrega de llaves.' }
      ]
    },
    'kaak-naajal': {
      title: 'Casa K\'áak Náajal',
      subtitle: 'Senderos Poniente Mz 12 Lt 08 · Ciudad Mayakoba',
      price: '$5,620,000 MXN',
      area: '342.10 m²',
      levels: 'Planta Baja + Nivel 1 + Roof Top Panorámico',
      pdfPlanos: '../assets/properties/kaak-naajal/docs/MANZANA_12_LT_8.pdf',
      pdfFicha: '../assets/properties/kaak-naajal/docs/MANZANA_12_LT_8.pdf',
      pdfBrochure: '../assets/properties/kaak-naajal/docs/MANZANA_12_LT_8.pdf',
      pdfCondiciones: '../assets/docs/ARUZ_CONDICIONES_DE_VENTA.pdf',
      specs: [
        { label: 'Superficie de Construcción', val: '342.10 m²' },
        { label: 'Superficie de Lote', val: '300.00 m²' },
        { label: 'Cimentación', val: 'Zapata Corrida sobre roca sólida CADE f\'c=250' },
        { label: 'Acabados', val: 'Mármol Travertino, Chukum, Tzalam y Granito Negro San Gabriel' }
      ],
      blueprintLevels: [
        { name: 'Planta Baja', desc: 'Acceso monumental, doble altura, sala, comedor, cocina con alacena, recámara de visitas con baño, alberca y terraza.' },
        { name: 'Primer Nivel', desc: 'Master Suite con terraza privada, baño con tina y vestidor amplio, 2 recámaras secundarias con baño en suite.' },
        { name: 'Roof Garden', desc: 'Cocina exterior, barra de granito, asador y jacuzzi con vista al atardecer.' }
      ],
      terms: [
        { title: 'Enganche', desc: '30% a la firma notarial.' },
        { title: 'Avance de Obra', desc: '60% distribuido en hitos de construcción.' },
        { title: 'Escritura', desc: '10% a la entrega formal.' }
      ]
    }
  };
  ...
  class AruzDocViewerModal {
    constructor() {
      ...
      this.activeTab = 'planos'; // 'planos' | 'ficha' | 'brochure' | 'condiciones'
    ...
    detectPropertyFromURL() {
      ...
      else if (path.includes('sak-luum')) this.currentPropKey = 'sak-luum';
      else if (path.includes('kaak-naajal')) this.currentPropKey = 'kaak-naajal';
    ...
    createModalDOM() {
      ...
      modalWrap.className = 'fixed inset-0 z-[99999] flex items-center justify-center p-3 md:p-6 opacity-0 pointer-events-none transition-all duration-300';
      modalWrap.innerHTML = `
        <!-- Backdrop -->
        <div id="aruz-doc-modal-backdrop" class="absolute inset-0 bg-black/85 backdrop-blur-xl transition-opacity duration-300"></div>

        <!-- Modal Window Container -->
        <div class="relative w-full max-w-5xl max-h-[92vh] bg-surface rounded-3xl border border-dorado-aruz/30 shadow-2xl overflow-hidden flex flex-col z-10 scale-95 transition-transform duration-300">
          
          <!-- Header Bar -->
          <div class="px-6 py-4 bg-carbon-aruz text-white flex items-center justify-between border-b border-white/10 shrink-0">
            <div class="flex items-center gap-3">
              <div class="w-9 h-9 rounded-xl bg-dorado-aruz/20 border border-dorado-aruz/40 flex items-center justify-center text-dorado-aruz">
                <span class="material-symbols-outlined text-xl" id="doc-modal-header-icon">architecture</span>
              </div>
              <div>
                <h3 id="doc-modal-title" class="font-serif text-lg md:text-xl font-bold text-dorado-aruz leading-tight">Casa Eternity Jol</h3>
                <p id="doc-modal-subtitle" class="text-xs text-white/60 font-sans">Senderos Norte Mz 18 Lt 03 · Ciudad Mayakoba</p>
              </div>
            </div>

            <!-- View Toggle & Close Button -->
            <div class="flex items-center gap-3">
              <div class="hidden sm:inline-flex bg-black/40 rounded-lg p-1 border border-white/10 text-xs">
                <button type="button" id="btn-mode-interactive" class="px-3 py-1 rounded font-bold text-dorado-aruz bg-white/10 transition-all">Vista Interactiva</button>
                <button type="button" id="btn-mode-pdf" class="px-3 py-1 rounded text-white/60 hover:text-white transition-all">Documento PDF</button>
              </div>
